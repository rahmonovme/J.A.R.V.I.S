...
class _BrowserThread:
    ...
    async def _close_browser(self) -> str:
        # Close Playwright's internal browser (if any)
        await self._close()

        # Also close the REAL system browser window
        import subprocess
        import platform
        import pyautogui
        import time
        ...
        if system == "Windows":
            ...
            for proc_name, display_name in browser_processes:
                try:
                    result = subprocess.run(
            ...
            # macOS: use osascript to quit browser apps
            mac_browsers = [
            ...
            for app_name, display_name in mac_browsers:
                try:
                    # Check if running
            ...
            if not closed:
                try:
                    ...
                    time.sleep(0.5)
                    return "Browser closed (Cmd+Q)."
                except Exception:
                    pass

        else:
            # Linux: pkill
            for proc in ["chrome", "chromium", "firefox", "brave", "opera"]:
                try:
                    result = subprocess.run(
                        ["pkill", "-f", proc],
                        capture_output=True, timeout=5
                    ...
                    if result.returncode == 0:
                        closed.append(proc.capitalize())
                except Exception:
                    pass

        if closed:
            return f"Browser closed ({', '.join(closed)})."
