...
def _send_whatsapp(receiver: str, message: str) -> str:
    ...
    try:
        ...
        # Open search (Ctrl+F works for WhatsApp Desktop search)
        pyautogui.hotkey("ctrl", "f")
        time.sleep(0.4)
        pyautogui.hotkey("ctrl", "a")

        # Use pyperclip for Unicode support
        if _PYPERCLIP:
...
def _send_telegram(receiver: str, message: str) -> str:
    ...
    Fixed: uses Escape→type for contact search (Telegram auto-focuses)."""
    try:
        ...
        # Escape to main screen first (important!)
        pyautogui.press("escape")
        time.sleep(0.3)
        pyautogui.press("escape")
        time.sleep(0.4)

        # In Telegram Desktop, typing on the main screen
        # auto-focuses the search bar and filters contacts
        if _PYPERCLIP:
            pyperclip.copy(receiver)
            pyautogui.hotkey("ctrl", "v")
        else:
